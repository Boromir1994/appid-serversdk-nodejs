...
function WebAppStrategy(options = {}) {
  ...
  this.serviceConfig = new ServiceUtil.loadConfig(
    "WebAppStrategy",
    [
      constants.CLIENT_ID,
      constants.TENANT_ID,
...
WebAppStrategy.prototype.setPreferredLocale = function (req, language) {
  ...
  if (!req.session) {
    logger.error(
      "Can't find req.session. Ensure express-session middleware is in use. Default language will be used"
    );
    return this.error(new Error("Can't find req.session"));
  }
  req.session.set(WebAppStrategy.LANGUAGE, language);
};

WebAppStrategy.prototype.getPreferredLocale = function (req) {
  return (
    (req.session && req.session.get(WebAppStrategy.LANGUAGE)) ||
    this.serviceConfig.getPreferredLocale()
  );
};
...
WebAppStrategy.prototype.authenticate = function (req, options = {}) {
  if (options.successRedirect) {
    req.session.set(returnTo, options.successRedirect);
  }

  // Check that express-session is enabled
  if (!req.session) {
    logger.error(
      "Can't find req.session. Ensure express-session middleware is in use"
    );
    return this.error(new Error("Can't find req.session"));
  }
  // RoP flow
...
WebAppStrategy.prototype.refreshTokens = function (req, refreshToken) {
  return refreshTokens(refreshToken, this).then(function (appIdAuthContext) {
    ...
    req.session.set(WebAppStrategy.AUTH_CONTEXT, appIdAuthContext);
  });
};
...
function handleChangePassword(req, options, strategy) {
  ...
  const appIdAuthContext = req.session.get(WebAppStrategy.AUTH_CONTEXT);
  if (
    !appIdAuthContext ||
    appIdAuthContext.identityTokenPayload["amr"][0] !== CLOUD_DIRECTORY
  ) {
    strategy.fail(
      new Error(
  ...
  req.session.set(WebAppStrategy.CLOUD_DIRECTORY_UPDATE_REQ, true);
...
function handleForgotPassword(req, options, strategy) {
  ...
  if (options.successRedirect) {
    req.session.set(WebAppStrategy.ORIGINAL_URL, options.successRedirect);
  }
...
function handleChangeDetails(req, options, strategy) {
  ...
  var generateCodeUrl =
    strategy.serviceConfig.getOAuthServerUrl() + GENERATE_CODE_PATH;
  request(
    {
      url: generateCodeUrl,
      auth: {
        bearer:
          appIdAuthContext.accessToken + " " + appIdAuthContext.identityToken,
      },
    },
    function (error, response, body) {
      if (!error) {
        if (response.statusCode === 200) {
          ...
          logger.debug(
            "handleChangeDetails :: redirecting to",
            changeDetailsUrl
          );
          req.session.set(WebAppStrategy.CLOUD_DIRECTORY_UPDATE_REQ, true);
          ...
          logger.error(
            "generate code response not 200, got status:" + response.statusCode
          );
          strategy.fail(
            new Error(
              "generate code: response status code:" + response.statusCode
            )
          );
        }
      } else {
...
function handleRopFlow(options, req, strategy) {
  ...
  if (
    appIdAuthContext &&
    appIdAuthContext.accessTokenPayload["amr"][0] === "appid_anon"
  ) {
  ...
  retrieveTokens(formData, strategy)
...
function handleAuthorization(req, options = {}, strategy) {
  ...
  if (
    isAuthenticated(req, options) &&
    !options.forceLogin &&
    !options.allowAnonymousLogin
  ) {
    return strategy.success(
      req.session.get(WebAppStrategy.AUTH_CONTEXT).identityTokenPayload
    );
  }

  // if successRedirect was specified, persist using passport's returnTo session param
  req.session.set(options.successRedirect || req.originalUrl);

  redirectForAuthorization(req, options, strategy);
}

function isAuthenticated(req, options = {}) {
  if (!req.session.get(WebAppStrategy.AUTH_CONTEXT)) {
    return false;
  }

  const allowExpiredTokensOnSession = options.hasOwnProperty(
    "allowExpiredTokensOnSession"
  )
    ? options.allowExpiredTokensOnSession
    : false;
  if (
    !allowExpiredTokensOnSession &&
    TokenUtil.isTokenExpired(
      req.session.get(WebAppStrategy.AUTH_CONTEXT).accessTokenPayload
    )
  ) {
    return false;
  }
...
function refreshTokens(refreshToken, strategy) {
  ...
  logger.debug(
    "obtain tokens using a refresh token, refreshToken=" + refreshToken
  );
  return retrieveTokens(formData, strategy);
}

function redirectForAuthorization(req, options, strategy) {
  options.allowCreateNewAnonymousUser = options.hasOwnProperty(
    "allowCreateNewAnonymousUser"
  )
    ? options.allowCreateNewAnonymousUser
    : true;
  ...
  if (
    !appIdAuthContext &&
    options.allowAnonymousLogin === true &&
    options.allowCreateNewAnonymousUser !== true
  ) {
    logger.info(
      "Previous anonymous user not found. Not allowed to create new anonymous users."
    );
  ...
  req.session.set(WebAppStrategy.STATE_PARAMETER, stateParameter);
...
function handleCallback(req, options = {}, strategy) {
  ...
  const stateParameter = req.session.get(WebAppStrategy.STATE_PARAMETER);
  // Check for handleChangeDetails, handleForgotPassword callback from cloud directory
  const cloudDirectoryUpdate = req.session.get(
    WebAppStrategy.CLOUD_DIRECTORY_UPDATE_REQ
  );
  if (
    typeof cloudDirectoryUpdate !== "undefined" &&
    cloudDirectoryUpdate === true
  ) {
    delete req.session[WebAppStrategy.CLOUD_DIRECTORY_UPDATE_REQ];
...
function generateForgotPasswordUrl(req, strategy) {
  ...
  const forgotPasswordEndpoint =
    serviceConfig.getOAuthServerUrl() + FORGOT_PASSWORD_PATH;
  var forgotPasswordUrl = encodeURI(
...
function generateChangePasswordUrl(userId, req, strategy) {
  ...
  const changePasswordEndpoint =
    serviceConfig.getOAuthServerUrl() + CHANGE_PASSWORD_PATH;
  const redirectUri = serviceConfig.getRedirectUri();
  var changePasswordUrl = encodeURI(
...
function generateChangeDetailsUrl(code, req, strategy) {
  const serviceConfig = strategy.serviceConfig;
  const clientId = serviceConfig.getClientId();
  const changeDetailsEndpoint =
    serviceConfig.getOAuthServerUrl() + CHANGE_DETAILS_PATH;
  const redirectUri = serviceConfig.getRedirectUri();
  var changeDetailsUrl = encodeURI(
    changeDetailsEndpoint +
      "?client_id=" +
      clientId +
      "&redirect_uri=" +
      redirectUri +
...
function generateAuthorizationUrl(options, req, strategy) {
  ...
  const clientId = serviceConfig.getClientId();
  const scope =
    WebAppStrategy.DEFAULT_SCOPE + (options.scope ? " " + options.scope : "");
  const authorizationEndpoint =
    serviceConfig.getOAuthServerUrl() + AUTHORIZATION_PATH;
  const redirectUri = serviceConfig.getRedirectUri();
      ...
      function (err, response, strBody) {
        if (err) {
          ...
          try {
            ...
            if (
              body &&
              (body.error === INVALID_GRANT || body["error_code"] === FORBIDDEN)
            ) {
...
function logAction(req, url, activity) {
  ...
  if (
    !appIdAuthContext ||
    !appIdAuthContext.accessToken ||
    !appIdAuthContext.identityToken
  ) {
...
WebAppStrategy.prototype.logoutSSO = function (req, res, options = {}) {
  ...
  let queryParams = Object.keys(options)
    .map((e) => encodeURIComponent(e) + "=" + encodeURIComponent(options[e]))
    .join("&");
...
WebAppStrategy.hasScope = function (req, requiredScopes) {
  ...
  if (
    !req ||
    !req.session ||
    !req.session.get(WebAppStrategy.AUTH_CONTEXT) ||
    !req.session.get(WebAppStrategy.AUTH_CONTEXT).accessTokenPayload ||
    typeof req.session.get(WebAppStrategy.AUTH_CONTEXT).accessTokenPayload
      .scope !== "string"
  ) {
  ...
  const suppliedScopes = req.session.get(WebAppStrategy.AUTH_CONTEXT)
    .accessTokenPayload.scope;
  // get the required scopes as an array
  const requiredScopesArray = requiredScopes
    .split(" ")
    .filter((scope) => scope !== ""); // split by spaces and ignore empty required scopes
  // get the supplied scopes as an array while removing the prefix (app URI) since the required scopes aren't prefixed:
  const suppliedScopesArray = suppliedScopes
    .split(" ")
    .map((fullScope) => fullScope.split("/").pop());
